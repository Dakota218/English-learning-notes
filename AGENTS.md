...
* 每次新增詞彙的詳細資訊（如中文意思、詞性、例句與補充說明），必須優先**從劍橋字典 (Cambridge Dictionary) 取得**，或是**直接使用 prompt 中所提供的資訊**。若劍橋字典上查無該詞彙的資訊，方可尋求其他權威詞典（例如 Merriam-Webster、Oxford 等）或來源。
* 如果 prompt 中僅提供單字列表而未給予詳細內容，必須主動透過網路搜尋劍橋字典的權威定義、真實詞性與自然例句，避免僅依賴 AI 自行生成。
