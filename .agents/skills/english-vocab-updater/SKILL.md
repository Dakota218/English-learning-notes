...
4. Parse the user's English input into clean learning items. Correct obvious spelling, grammar, or usage mistakes and mention important corrections in the final report.
   ...
   - When an expression ends with a parenthetical gloss, such as `kneel (跪下)` or `kneel（跪下）`, treat the parenthetical text as the user-provided Chinese meaning. Remove the parentheses and gloss from the stored expression, and use the gloss for `meaning_zh`. This convention may be combined with the colon example rule and the period fixed-expression rule.
